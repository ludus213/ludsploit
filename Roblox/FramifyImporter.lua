...
local AUTO_CENTER_UI = true -- New: Automatically center the imported UI
...
mainFrame.BackgroundColor3 = Color3.fromRGB(44, 44, 44) -- Darker background
...
-- Scrolling frame for the text box to prevent clipping
...
mappingTextBox.Size = UDim2.new(1, 0, 1, 0) -- Text box fills the scroll frame
mappingTextBox.AutomaticSize = Enum.AutomaticSize.Y -- Expands vertically with text
...
textScrollFrame.CanvasSize = UDim2.new(0,0,0,0) -- Will be controlled by content size
...
btnCorner.Parent = importGuiButton
...
--------------------------------------------------------------------------------
--[[ CORE LOGIC (functions are the same, only the final import logic changes) ]]--
--------------------------------------------------------------------------------
...
function findImageAsset(assetId) if not assetId then return "" end local assetLocation = IMAGE_ASSET_LOCATION or ReplicatedStorage; local image = assetLocation:FindFirstChild(assetId, true) if image then if image:IsA("ImageLabel") then return image.Image elseif image:IsA("ImageButton") then return image.Image elseif image:IsA("Decal") then return image.Texture end end if tonumber(assetId) then return "rbxassetid://" .. assetId end return "" end
...
importGuiButton.MouseButton1Click:Connect(function()
    ...
    if AUTO_CENTER_UI then
        ...
        for _, nodeData in ipairs(data) do
            ...
            minX = math.min(minX, props.position.x)
            minY = math.min(minY, props.position.y)
            maxX = math.max(maxX, props.position.x + props.size.x)
            maxY = math.max(maxY, props.position.y + props.size.y)
        end
        mainContainer.Size = UDim2.fromOffset(maxX - minX, maxY - minY)

        -- Adjust children positions relative to the new container
        for _, nodeData in ipairs(data) do
            nodeData.properties.position.x = nodeData.properties.position.x - minX
            nodeData.properties.position.y = nodeData.properties.position.y - minY
        end
    ...
    for _, nodeData in ipairs(data) do
        createFromData(nodeData, importParent)
    end
