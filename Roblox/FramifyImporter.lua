-- Framify Importer
-- Version: 5.0.0 DEFINITIVE FINAL
...
function applyTheme(themeName)
    ...
    for name, element in pairs(UI) do
        ...
        elseif name:match("Button") and element:IsA("TextButton") then element.BackgroundColor3 = theme.Primary; element.TextColor3 = Color3.new(1,1,1)
        elseif name:match("Dropdown") or name:match("Check") then element.BackgroundColor3 = theme.Surface; element.TextColor3 = theme.Text
        elseif name:match("Icon") then element.ImageColor3 = theme.TextSecondary
        end
        if UI.PromptYes then UI.PromptYes.BackgroundColor3 = theme.Primary end
        if UI.PromptNo then UI.PromptNo.BackgroundColor3 = theme.Danger end
    end
end
...
function createMainUI(widget)
    ...
    local l = Instance.new("UIListLayout",f); l.Padding=UDim.new(0,15); l.SortOrder=Enum.SortOrder.LayoutOrder
    local t = Instance.new("TextLabel",f); t.Name="Title"; t.LayoutOrder=1; t.Text="Framify Importer"; t.Size=UDim2.new(1,0,0,24); t.Font=Enum.Font.GothamBold; t.TextSize=22; t.BackgroundTransparency=1; t.TextXAlignment=Enum.TextXAlignment.Left; UI.MainTitle=t
    local i = Instance.new("TextLabel",f); i.Name="Instructions"; i.LayoutOrder=2; i.Text="Paste your mapping string below to begin."; i.Size=UDim2.new(1,0,0,18); i.Font=Enum.Font.Gotham; i.TextSize=14; i.TextWrapped=true; i.BackgroundTransparency=1; i.TextXAlignment=Enum.TextXAlignment.Left; UI.MainInstructions=i
    ...
    local btn = Instance.new("TextButton",f); btn.Name="Button"; btn.LayoutOrder=4; btn.Text="Import"; btn.Font=Enum.Font.GothamBold; btn.TextSize=16; btn.Size=UDim2.new(1,0,0,40); UI.MainImportButton=btn; Instance.new("UICorner",btn).CornerRadius=UDim.new(0,6)
    local st = Instance.new("TextLabel",f); st.Name="Label"; st.LayoutOrder=5; st.Text=""; st.Font=Enum.Font.Gotham; st.TextSize=12; st.BackgroundTransparency=1; st.TextXAlignment=Enum.TextXAlignment.Left; st.Size=UDim2.new(1,0,0,20); UI.MainStatusLabel=st
    local sup = Instance.new("TextLabel",f); sup.Name="Support"; sup.LayoutOrder=6; sup.Text="Contact .ludio. on Discord for support/errors"; sup.Font=Enum.Font.Gotham; sup.TextSize=10; sup.BackgroundTransparency=1; sup.Size=UDim2.new(1,0,0,20); UI.MainSupportLabel=sup
    return btn, tb, st
end
...
function createSettingsUI(widget)
    ...
    local d = Instance.new("TextButton",df); d.Name="Dropdown"; d.Size=UDim2.fromScale(1,1); d.Text=Config.THEME; UI.SettingsThemeDropdown=d
    local o = Instance.new("ScrollingFrame",df); o.Name="OptionsFrame"; o.Position=UDim2.new(0,0,1,0); o.Size=UDim2.new(1,0,0,120); o.Visible=false; o.Parent=d
    local ol = Instance.new("UIListLayout",o)
    d.MouseButton1Click:Connect(function() o.Visible=not o.Visible end)
    for name,_ in pairs(Themes) do local b=Instance.new("TextButton",o); b.Name="Dropdown"; b.Size=UDim2.new(1,0,0,30); b.Text=name; b.MouseButton1Click:Connect(function() d.Text=name; o.Visible=false; applyTheme(name) end) end
end
...
function populatePromptUI(widget, title, text, onYes, onNo)
    ...
    local p = Instance.new("UIPadding",f); p.PaddingLeft,p.PaddingRight,p.PaddingTop,p.PaddingBottom=UDim.new(0,15),UDim.new(0,15),UDim.new(0,15),UDim.new(0,15)
    local l = Instance.new("UIListLayout",f); l.Padding=UDim.new(0,15); l.SortOrder=Enum.SortOrder.LayoutOrder; l.VerticalAlignment=Enum.VerticalAlignment.Center
    local t = Instance.new("TextLabel",f); t.Name="Title"; t.LayoutOrder=1; t.Text=title; t.Size=UDim2.new(1,0,0,24); t.Font=Enum.Font.GothamBold; t.TextSize=22; t.BackgroundTransparency=1; t.TextXAlignment=Enum.TextXAlignment.Left; UI.PromptTitle=t
    local i = Instance.new("TextLabel",f); i.Name="Label"; i.LayoutOrder=2; i.Text=text; i.Size=UDim2.new(1,0,0,60); i.Font=Enum.Font.Gotham; i.TextSize=14; i.TextWrapped=true; i.BackgroundTransparency=1; i.TextXAlignment=Enum.TextXAlignment.Left; UI.PromptText=i
    ...
    local bl = Instance.new("UIListLayout",bf); bl.FillDirection=Enum.FillDirection.Horizontal; bl.HorizontalAlignment=Enum.HorizontalAlignment.Right; bl.Padding=UDim.new(0,10)
...
function findImageAsset(assetId) if not assetId then return "" end; local assetFolder = ReplicatedStorage:FindFirstChild(Config.ASSET_FOLDER_NAME); if not assetFolder then return nil end; local image = assetFolder:FindFirstChild(assetId, true); if image then if image:IsA("ImageLabel") then return image.Image elseif image:IsA("ImageButton") then return image.Image elseif image:IsA("Decal") then return image.Texture end end; return nil end
...
function verifyAssets(assetIds) local missing = {}; local assetFolder = ReplicatedStorage:FindFirstChild(Config.ASSET_FOLDER_NAME); if not assetFolder then return assetIds end; for _,id in ipairs(assetIds) do if not assetFolder:FindFirstChild(id) then table.insert(missing, id) end end; return missing end
...
local promptWidget = plugin:CreateDockWidgetPluginGui("FramifyPrompt", DockWidgetPluginGuiInfo.new(Enum.InitialDockState.Float, true, false, 380, 220, 380, 220))
...
importBtn.MouseButton1Click:Connect(function()
    ...
    if #requiredAssets > 0 then
        local missingAssets = verifyAssets(requiredAssets)
        if #missingAssets > 0 then
            statusLabel.Text = "Error: Missing images: " .. table.concat(missingAssets, ", ")
            return
        end
